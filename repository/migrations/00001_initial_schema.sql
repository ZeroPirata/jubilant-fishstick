...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'user_skills_user_id_skill_name_key'
    ) THEN
        ALTER TABLE user_skills ADD CONSTRAINT user_skills_user_id_skill_name_key UNIQUE (user_id, skill_name);
    END IF;
END $$;
-- +goose StatementEnd
...
DROP TABLE IF EXISTS user_job_filters;
DROP TABLE IF EXISTS resumes_feedback;
DROP TABLE IF EXISTS generated_resumes;
DROP TABLE IF EXISTS jobs;
DROP TABLE IF EXISTS stack_aliases;
DROP TABLE IF EXISTS user_certificates;
DROP TABLE IF EXISTS user_projects;
DROP TABLE IF EXISTS user_skills;
DROP TABLE IF EXISTS user_academic_histories;
